...
class BB_Akismet {
	...
	private function setup_actions() {
		...
		$checks['check']  = array(
			'bb_new_topic_pre_insert'  => 1,  // New topic check
			'bb_new_reply_pre_insert'  => 1,  // New reply check
			'bb_edit_topic_pre_insert' => 1,  // Edit topic check
			'bb_edit_reply_pre_insert' => 1   // Edit reply check
		);

		// bbPress functions for spam and ham submissions
		$checks['submit'] = array(
			'bb_spammed_topic'   => 10, // Spammed topic
			'bb_unspammed_topic' => 10, // Unspammed reply
			'bb_spammed_reply'   => 10, // Spammed reply
			'bb_unspammed_reply' => 10, // Unspammed reply
		);
	 ...
	 * @uses bb_filter_anonymous_user_data() To get anonymous user data
	 * @uses bb_get_topic_permalink() To get the permalink of the topic
	 * @uses bb_get_reply_url() To get the permalink of the reply
	 * @uses bb_current_author_ip() To get the IP address of the current user
	 * @uses BB_Akismet::maybe_spam() To check if post is spam
	 * @uses BB_Akismet::get_user_roles() To get the role(s) of the current user
	 * @uses do_action() To call the 'bb_akismet_spam_caught' hook
	 * @uses add_filter() To call the 'bb_new_reply_pre_set_terms' hook
	 *
	...
	public function check_post( $post_data ) {
		...
		if ( bb_get_public_status_id() != $post_data['post_status'] )
			return $post_data;
		...
		if ( !empty( $anonymous_data ) ) {
			$user_data['name']    = $anonymous_data['bb_anonymous_name'];
			$user_data['email']   = $anonymous_data['bb_anonymous_name'];
			$user_data['website'] = $anonymous_data['bb_anonymous_name'];
		...
		// Get the result
		$post_data['bb_akismet_result'] = $_post['bb_akismet_result'];
		unset( $_post['bb_akismet_result'] );

		// Store the data as submitted
		$post_data['bb_post_as_submitted'] = $_post;

		// Allow post_data to be manipulated
		do_action_ref_array( 'bb_akismet_check_post', $post_data );

		// Spam
		if ( 'true' == $post_data['bb_akismet_result'] ) {

			// Let plugins do their thing
			do_action( 'bb_akismet_spam_caught' );

			// This is spam
			$post_data['post_status'] = bb_get_spam_status_id();

			// We don't want your spam tags here
			add_filter( 'bb_new_reply_pre_set_terms', array( $this, 'filter_post_terms' ), 1, 3 );
	 ...
	 * @uses bb_get_user_profile_url() To get a user's profile url
	 * @uses get_permalink() To get the permalink of the post_parent
	 * @uses BB_Akismet::get_user_roles() To get the role(s) of the post_author
	 * @uses bb_current_author_ip() To get the IP address of the current user
	 * @uses BB_Akismet::maybe_spam() To submit the post as ham or spam
	 * @uses update_post_meta() To update the post meta with some Akismet data
	 * @uses do_action() To call the 'bb_akismet_submit_spam_post' and 'bb_akismet_submit_ham_post' hooks
	 *
	...
	public function submit_post( $post_id = 0 ) {
		...
		switch ( $current_filter ) {
			...
			case 'bb_spammed_topic' :
			case 'bb_spammed_reply' :
				$request_type = 'spam';
				break;

			// Honey-glazed, a straight off the bone
			case 'bb_unspammed_topic' :
			case 'bb_unspammed_reply' :
				$request_type = 'ham';
				break;
		...
		if ( ( 'spam' == $request_type ) && ( bb_get_spam_status_id() != $_post->post_status ) )
			return;

		// Set some default post_data
		$post_data = array(
			'comment_approved'     => $_post->post_status,
			'comment_author'       => $_post->post_author ? get_the_author_meta( 'display_name', $_post->post_author ) : get_post_meta( $post_id, '_bb_anonymous_name',    true ),
			'comment_author_email' => $_post->post_author ? get_the_author_meta( 'email',        $_post->post_author ) : get_post_meta( $post_id, '_bb_anonymous_email',   true ),
			'comment_author_url'   => $_post->post_author ? bb_get_user_profile_url(            $_post->post_author ) : get_post_meta( $post_id, '_bb_anonymous_website', true ),
		...
		if ( isset( $post_data['reporter'] ) ) {
			...
			switch ( $request_type ) {
				...
				case 'spam' :
					...
					update_post_meta( $post_id, '_bb_akismet_user_result', 'true'                 );
					update_post_meta( $post_id, '_bb_akismet_user',        $post_data['reporter'] );
					break;
				...
				case 'ham'  :
					...
					update_post_meta( $post_id, '_bb_akismet_user_result', 'false'                 );
					update_post_meta( $post_id, '_bb_akismet_user',         $post_data['reporter'] );
		...
		do_action( 'bb_akismet_submit_' . $request_type . '_post', $post_id, $post_data['bb_akismet_result'] );
	}
	...
	private function maybe_spam( $post_data, $check = 'check', $spam = 'spam' ) {
		...
		if ( !empty( $response[1] ) ) {
			$post_data['bb_akismet_result'] = $response[1];
		} else {
			$post_data['bb_akismet_result'] = __( 'No response', 'bbpress' );
		}
	 ...
	 * @uses BB_Akismet::update_post_history() To update post Akismet history
	 */
	public function update_post_meta( $post_id = 0, $_post = false ) {
		...
		if ( !empty( $this->last_post ) )
			$as_submitted = $this->last_post['bb_post_as_submitted'];

		// wp_insert_post() might be called in other contexts. Ensure this is
		// the same topic/reply as was checked by BB_Akismet::check_post()
		if ( is_object( $_post ) && !empty( $this->last_post ) && is_array( $as_submitted ) ) {
			...
			if (	intval( $as_submitted['comment_post_ID'] )    == intval( $_post->post_parent )
					&&      $as_submitted['comment_author']       == ( $anonymous_data ? $anonymous_data['bb_anonymous_name']  : $userdata->display_name )
					&&      $as_submitted['comment_author_email'] == ( $anonymous_data ? $anonymous_data['bb_anonymous_email'] : $userdata->user_email   )
				) {

				// Normal result: true
				if ( $this->last_post['bb_akismet_result'] == 'true' ) {

					// Leave a trail so other's know what we did
					update_post_meta( $post_id, '_bb_akismet_result', 'true' );
					...
					if ( $_post->post_status != bb_get_spam_status_id() ) {
				...
				// Normal result: false
				} elseif ( $this->last_post['bb_akismet_result'] == 'false' ) {

					// Leave a trail so other's know what we did
					update_post_meta( $post_id, '_bb_akismet_result', 'false' );
				...
				// Abnormal result: error
					...
					// Leave a trail so other's know what we did
					update_post_meta( $post_id, '_bb_akismet_error', time() );
					$this->update_post_history( $post_id, sprintf( __( 'Akismet was unable to check this post (response: %s), will automatically retry again later.', 'bbpress' ), $this->last_post['bb_akismet_result'] ), 'check-error' );
				}

				// Record the complete original data as submitted for checking
				if ( isset( $this->last_post['bb_post_as_submitted'] ) ) {
					update_post_meta( $post_id, '_bb_akismet_as_submitted', $this->last_post['bb_post_as_submitted'] );
				}
			}
		}
	}
	...
	private function update_post_history( $post_id = 0, $message = null, $event = null ) {
		...
		// Save the event data
		add_post_meta( $post_id, '_bb_akismet_history', $event );
	}
	...
	public function get_post_history( $post_id = 0 ) {
		...
		$history = get_post_meta( $post_id, '_bb_akismet_history' );
	 ...
	 * @param int $reply_id
	 *
	 * @uses bb_get_reply_id() To get the reply_id
	 * @uses bb_get_topic_id() To get the topic_id
	...
	public function filter_post_terms( $terms = '', $topic_id = 0, $reply_id = 0 ) {
		...
		$existing_terms = wp_get_object_terms( $topic_id, bb_get_topic_tag_tax_id(), array( 'fields' => 'names' ) );

		// Save the terms for later in case the reply gets hammed
		if ( !empty( $terms ) )
			update_post_meta( $reply_id, '_bb_akismet_spam_terms', $terms );
