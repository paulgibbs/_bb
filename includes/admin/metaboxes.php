 ...
 * @uses do_action() Calls 'bb_dashboard_widget_right_now_content_table_end'
 *                    below the content table
 * @uses do_action() Calls 'bb_dashboard_widget_right_now_table_end'
 *                    below the discussion table
 * @uses do_action() Calls 'bb_dashboard_widget_right_now_discussion_table_end'
 *                    below the discussion table
 * @uses do_action() Calls 'bb_dashboard_widget_right_now_end' below the widget
 */
function bb_dashboard_widget_right_now() {
	...
	<div class="table table_content">
		...
		<table>
			...
			<tr class="first">
				...
				<?php
					...
					if ( current_user_can( 'publish_forums' ) ) {
						$link = add_query_arg( array( 'post_type' => bb_get_forum_post_type() ), get_admin_url( null, 'edit.php' ) );
			...
			<tr>
				...
				<?php
					...
					if ( current_user_can( 'publish_topics' ) ) {
						$link = add_query_arg( array( 'post_type' => bb_get_topic_post_type() ), get_admin_url( null, 'edit.php' ) );
						$num  = '<a href="' . $link . '">' . $num  . '</a>';
						$text = '<a href="' . $link . '">' . $text . '</a>';
					}
				?>
			...
			<tr>
				...
				<?php
					...
					if ( current_user_can( 'publish_replies' ) ) {
						$link = add_query_arg( array( 'post_type' => bb_get_reply_post_type() ), get_admin_url( null, 'edit.php' ) );
						$num  = '<a href="' . $link . '">' . $num  . '</a>';
						$text = '<a href="' . $link . '">' . $text . '</a>';
					}
				?>
				...
				<tr>
					...
					<?php
						...
						if ( current_user_can( 'manage_topic_tags' ) ) {
							$link = add_query_arg( array( 'taxonomy' => bb_get_topic_tag_tax_id(), 'post_type' => bb_get_topic_post_type() ), get_admin_url( null, 'edit-tags.php' ) );
							$num  = '<a href="' . $link . '">' . $num  . '</a>';
							$text = '<a href="' . $link . '">' . $text . '</a>';
						}
					?>
			...
			<?php do_action( 'bb_dashboard_widget_right_now_content_table_end' ); ?>
	...
	<div class="table table_discussion">
		...
		<table>
			...
			<?php if ( isset( $r['topic_count_hidden'] ) ) : ?>
				...
				<tr>
					...
					<?php
						...
						$link = add_query_arg( array( 'post_type' => bb_get_topic_post_type() ), get_admin_url( null, 'edit.php' ) );
						if ( '0' != $num ) {
							$link = add_query_arg( array( 'post_status' => bb_get_spam_status_id() ), $link );
						}
			...
			<?php if ( bb_allow_topic_tags() && isset( $r['empty_topic_tag_count'] ) ) : ?>
			...
			<?php do_action( 'bb_dashboard_widget_right_now_discussion_table_end' ); ?>

		</table>

	</div>

	<?php do_action( 'bb_dashboard_widget_right_now_table_end' ); ?>

	<div class="versions">

		<span id="wp-version-message">
			<?php printf( __( 'You are using <span class="b">bbPress %s</span>.', 'bbpress' ), bb_get_version() ); ?>
		</span>
	...
	do_action( 'bb_dashboard_widget_right_now_end' );
}
 ...
 * @uses bb_is_forum_private() To check if a forum is private or not
 * @uses bb_dropdown() To show a dropdown of the forums for forum parent
 * @uses do_action() Calls 'bb_forum_metabox'
 */
function bb_forum_metabox() {

	// Post ID
	$post_id     = get_the_ID();
	$post_parent = bb_get_global_post_field( 'post_parent', 'raw'  );
	$menu_order  = bb_get_global_post_field( 'menu_order',  'edit' );
	...
	<p>
		...
		<label class="screen-reader-text" for="bb_forum_type_select"><?php _e( 'Type:', 'bbpress' ) ?></label>
		<?php bb_form_forum_type_dropdown( $post_id ); ?>
	</p>
	...
	<p>
		...
		<label class="screen-reader-text" for="bb_forum_status_select"><?php _e( 'Status:', 'bbpress' ) ?></label>
		<?php bb_form_forum_status_dropdown( $post_id ); ?>
	</p>
	...
	<p>
		...
		<label class="screen-reader-text" for="bb_forum_visibility_select"><?php _e( 'Visibility:', 'bbpress' ) ?></label>
		<?php bb_form_forum_visibility_dropdown( $post_id ); ?>
	</p>
	...
	<?php
	wp_nonce_field( 'bb_forum_metabox_save', 'bb_forum_metabox' );
	do_action( 'bb_forum_metabox', $post_id );
}
...
function bb_topic_metabox() {
	...
	<p>
		<strong class="label"><?php _e( 'Type:', 'bbpress' ); ?></strong>
		<label class="screen-reader-text" for="bb_stick_topic"><?php _e( 'Topic Type', 'bbpress' ); ?></label>
		<?php bb_topic_type_select( array( 'topic_id' => $post_id ) ); ?>
	</p>
	...
	<p>
		...
		<?php bb_dropdown( array(
			'post_type'          => bb_get_forum_post_type(),
			'selected'           => bb_get_topic_forum_id( $post_id ),
			'numberposts'        => -1,
			'orderby'            => 'title',
			'order'              => 'ASC',
			'walker'             => '',
	...
	<?php
	wp_nonce_field( 'bb_topic_metabox_save', 'bb_topic_metabox' );
	do_action( 'bb_topic_metabox', $post_id );
}
...
function bb_reply_metabox() {
	...
	// Get some meta
	$reply_topic_id = bb_get_reply_topic_id( $post_id );
	$reply_forum_id = bb_get_reply_forum_id( $post_id );
	...
	if ( current_user_can( 'edit_others_replies' ) || current_user_can( 'moderate' ) ) : ?>
		...
		<p>
			<strong class="label"><?php _e( 'Forum:', 'bbpress' ); ?></strong>
			<label class="screen-reader-text" for="bb_forum_id"><?php _e( 'Forum', 'bbpress' ); ?></label>
			<?php bb_dropdown( array(
				'post_type'          => bb_get_forum_post_type(),
	...
	<p>
		...
		<input name="parent_id" id="bb_topic_id" type="text" value="<?php echo esc_attr( $reply_topic_id ); ?>" />
	</p>

	<?php
	wp_nonce_field( 'bb_reply_metabox_save', 'bb_reply_metabox' );
	do_action( 'bb_reply_metabox', $post_id );
}
...
function bb_author_metabox() {
	...
	if ( bb_is_reply_anonymous( $post_id ) || bb_is_topic_anonymous( $post_id ) ) : ?>

		<p>
			<strong class="label"><?php _e( 'Name:', 'bbpress' ); ?></strong>
			<label class="screen-reader-text" for="bb_anonymous_name"><?php _e( 'Name', 'bbpress' ); ?></label>
			<input type="text" id="bb_anonymous_name" name="bb_anonymous_name" value="<?php echo esc_attr( get_post_meta( $post_id, '_bb_anonymous_name', true ) ); ?>" />
		</p>

		<p>
			<strong class="label"><?php _e( 'Email:', 'bbpress' ); ?></strong>
			<label class="screen-reader-text" for="bb_anonymous_email"><?php _e( 'Email', 'bbpress' ); ?></label>
			<input type="text" id="bb_anonymous_email" name="bb_anonymous_email" value="<?php echo esc_attr( get_post_meta( $post_id, '_bb_anonymous_email', true ) ); ?>" />
		</p>

		<p>
			<strong class="label"><?php _e( 'Website:', 'bbpress' ); ?></strong>
			<label class="screen-reader-text" for="bb_anonymous_website"><?php _e( 'Website', 'bbpress' ); ?></label>
			<input type="text" id="bb_anonymous_website" name="bb_anonymous_website" value="<?php echo esc_attr( get_post_meta( $post_id, '_bb_anonymous_website', true ) ); ?>" />
		</p>
	...
	<p>
		...
		<label class="screen-reader-text" for="bb_author_ip_address"><?php _e( 'IP Address', 'bbpress' ); ?></label>
		<input type="text" id="bb_author_ip_address" name="bb_author_ip_address" value="<?php echo esc_attr( get_post_meta( $post_id, '_bb_author_ip', true ) ); ?>" disabled="disabled" />
	</p>

	<?php

	do_action( 'bb_author_metabox', $post_id );
}
