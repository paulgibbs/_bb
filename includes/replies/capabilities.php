 ...
 * @since barebones (1.0)
 *
 * @uses apply_filters() Calls 'bb_get_reply_caps' with the capabilities
 * @return array Reply capabilities
 */
function bb_get_reply_caps() {
	return apply_filters( 'bb_get_reply_caps', array (
...
function bb_map_reply_meta_caps( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {
	...
	switch ( $cap ) {
		...
		case 'bb_replies_admin' :
			$caps = array( 'moderate' );
			break;
	}

	return apply_filters( 'bb_map_reply_meta_caps', $caps, $cap, $user_id, $args );
}
