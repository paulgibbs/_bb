...
<div id="bbpress-forums">
	...
	<?php if ( is_user_logged_in() && current_user_can( 'edit_topic', bb_get_topic_id() ) ) : ?>
		...
		<div id="merge-topic-<?php bb_topic_id(); ?>" class="bbp-topic-merge">

			<form id="merge_topic" name="merge_topic" method="post" action="<?php the_permalink(); ?>">

				<fieldset class="bbp-form">

					<legend><?php printf( __( 'Merge topic "%s"', 'bbpress' ), bb_get_topic_title() ); ?></legend>
						...
						<fieldset class="bbp-form">
							...
							<div>
								<?php if ( bb_has_topics( array( 'show_stickies' => false, 'post_parent' => bb_get_topic_forum_id( bb_get_topic_id() ), 'post__not_in' => array( bb_get_topic_id() ) ) ) ) : ?>
									...
									<label for="bb_destination_topic"><?php _e( 'Merge with this topic:', 'bbpress' ); ?></label>
						...
						<fieldset class="bbp-form">
							...
							<div>
								...
								<?php if ( bb_is_subscriptions_active() ) : ?>
									...
									<label for="bb_topic_subscribers"><?php _e( 'Merge topic subscribers', 'bbpress' ); ?></label><br />

								<?php endif; ?>

								<input name="bb_topic_favoriters" id="bb_topic_favoriters" type="checkbox" value="1" checked="checked" tabindex="<?php bb_tab_index(); ?>" />
								<label for="bb_topic_favoriters"><?php _e( 'Merge topic favoriters', 'bbpress' ); ?></label><br />
								...
								<?php if ( bb_allow_topic_tags() ) : ?>
									...
									<label for="bb_topic_tags"><?php _e( 'Merge topic tags', 'bbpress' ); ?></label><br />
					...
					<?php bb_merge_topic_form_fields(); ?>
