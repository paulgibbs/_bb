jQuery( document ).ready( function ( $ ) {
	...
	function bb_ajax_call( action, topic_id, nonce, update_selector ) {
		...
		$.post( bbpTopicJS.bb_ajaxurl, $data, function ( response ) {
			if ( response.success ) {
	...
	$( '#favorite-toggle' ).on( 'click', 'span a.favorite-toggle', function( e ) {
		e.preventDefault();
		bb_ajax_call( 'favorite', $( this ).attr( 'data-topic' ), bbpTopicJS.fav_nonce, '#favorite-toggle' );
	} );

	$( '#subscription-toggle' ).on( 'click', 'span a.subscription-toggle', function( e ) {
		e.preventDefault();
		bb_ajax_call( 'subscription', $( this ).attr( 'data-topic' ), bbpTopicJS.subs_nonce, '#subscription-toggle' );
	} );
} );
